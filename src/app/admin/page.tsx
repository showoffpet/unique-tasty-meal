...
// Mock Data targeting 48-hour batch model
const METRICS = [
  {
    title: "Total Revenue",
    value: "$1,240.50",
...
const POPULAR_ITEMS = [
  { name: "Jollof Rice", count: 142, max: 150, image: "/images/jollof.jpg" },
  { name: "Suya Platter", count: 98, max: 150, image: "/images/suya.jpg" },
  { name: "Pounded Yam", count: 76, max: 150, image: "/images/yam.jpg" },
  { name: "Egusi Soup", count: 54, max: 150, image: "/images/egusi.jpg" },
];
      ...
      <main className="flex-1 p-8 overflow-y-auto pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          ...
          <div className="lg:col-span-2 bg-[#ffffff] rounded-xl border border-[#f3f1f1] p-6 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-sm text-[#806b6b]">
                  Income trend over last 24 hours
                </p>
              </div>
            ...
            <div className="h-64 flex items-end justify-between gap-2 mt-8 px-2">
              {[4, 6, 3, 7, 5, 9, 6, 4, 3, 5, 7, 8].map((h, i) => (
                <div
                  key={i}
                  className="w-full bg-[#f3f1f1] rounded-t-sm relative group h-full flex flex-col justify-end"
                >
                  <div
                    className={`absolute bottom-0 w-full rounded-t-sm transition-colors ${
                      i === 5
                        ? "bg-[#7b2d2d] group-hover:bg-[#561b1b]"
                        : "bg-[#7b2d2d]/20 group-hover:bg-[#7b2d2d]/30"
                    }`}
                    style={{ height: `${h * 10}%` }}
                  />
                </div>
          ...
          <div className="lg:col-span-1 bg-[#ffffff] rounded-xl border border-[#f3f1f1] p-6 shadow-sm flex flex-col">
            ...
            <p className="text-sm text-[#806b6b] mb-6">
                ...
                <div key={i} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-end mb-1">
                      <span className="text-sm font-semibold text-[#1e1414] truncate">
                        {item.name}
                      </span>
                    ...
                    <div className="h-1.5 w-full bg-[#f3f1f1] rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[#7b2d2d] rounded-full"
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#f3f1f1] text-sm">
                {LIVE_ORDERS.map((order, i) => (
                  <tr
                    key={i}
                    className="hover:bg-[#f3f1f1]/30 transition-colors group"
                  >
                    <td className="p-4 font-mono font-medium text-[#1e1414]">
                      {order.id}
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-[#806b6b] max-w-[200px] truncate">
                      {order.items}
                    </td>
